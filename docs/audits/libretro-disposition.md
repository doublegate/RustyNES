...
| L-1.2 | `on_load_game` ignores `_game` and requires `GET_GAME_INFO_EXT` (§1.2, Fix 10) | CONFIRMED; BLOCKED upstream | `lib.rs:855` takes `_game` and `:899` refuses a frontend without `GET_GAME_INFO_EXT`. **The fallback cannot be written against `rust-libretro` 0.3.2**: libretro.h forward-declares `struct retro_game_info;` (line 2879) before defining it (line 3661), bindgen emits an opaque `{ _address: u8 }`, and `retro_load_game` passes `Some(*game)`, one byte, so the core never sees `path`/`data`/`size`. The limit was already known: `docs/libretro/WALKTHROUGH.md:26` records the opaque struct as the reason the core uses `GET_GAME_INFO_EXT` at all, so the audit's "ignores `_game`" is its consequence, not an oversight. Red test `abi_tests::a_frontend_without_game_info_ext_still_loads_the_game` (ignored with the reason). Needs a vendored patch or an upstream fix: maintainer decision | v2.8.0 | |
| L-1.3 | Memory maps not deregistered on unload (§1.3, Fix 5) | CONFIRMED | RetroArch keeps a core's memory maps until the core itself is torn down (`runloop.c` `uninit_libretro_symbols` -> `runloop_system_info_free`), not on content unload, so after `retro_unload_game` its descriptors point at freed WRAM/SRAM/CIRAM. An empty map clears them: the `SET_MEMORY_MAPS` handler frees the old descriptors before reading the new count (`runloop.c:3058-3079`). Red test `abi_tests::unloading_withdraws_the_memory_maps` (last map had 3 descriptors after unload) | v2.8.0 | |
...
| L-2.1 | `serialize_size` fixed at load; attaching a Zapper grows the snapshot (§2.1, Fix 3) | CONFIRMED | `serialize_size` assigned only at `lib.rs:971/979`; `set_zapper` attaches the device lazily; the bus section grows 6 bytes per Zapper; `on_serialize` returns false once the slice is short. Red test `abi_tests::save_states_fit_with_a_zapper_on_both_ports_and_restore_padded` fails at the serialize | v2.8.0 | |
| L-2.2 | `SectionIter` rejects host trailing padding (§2.2, Fix 4) | FIXED (core half) | `SectionIter` ends at an all-zero tail (tags are pinned zero-free); any non-zero byte is still parsed. Red first: one zero byte of padding gave `Eof`. Tests `section_iter_ends_at_an_all_zero_tail`, `restore_accepts_a_snapshot_followed_by_zero_padding`. The libretro half (zeroing the tail on serialize) lands with L-2.1 | v2.8.0 | |
