...
| SEC-06 | Medium | FDS disk saves, RetroAchievements progress, and TAS `.rnm` movies use bare `std::fs::write` instead of `write_atomic`. | FIXED | CONFIRMED, and wider than reported: besides the FDS save (`emu.rs`), the RA blob and movies, the screenshot, SRT, history-clip, fm2/bk2 export and TAStudio-project writes in `app.rs`, the memory-compare export and both `hdpack_builder.rs` writes were bare. All now use `write_atomic`. The one left bare is the CPU trace dump in the OS temp dir (regenerated on demand, nothing user-authored). Guard: `crates/rustynes-frontend/tests/no_bare_fs_write.rs` fails on any production `fs::write(` outside an argued allow-list, and on a stale allow-list entry; mutation (reintroduce the FDS bare write) CAUGHT | v2.7.1 | |
