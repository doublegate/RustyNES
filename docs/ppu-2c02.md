...
2. **PPUMASK rendering toggle mid-screen.** Disabling rendering corrupts 1 OAM row. Turning it off mid-screen with `v` in `$3C00-$3FFF` corrupts palette RAM (mapper hardware races with the address bus).
