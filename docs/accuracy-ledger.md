...
| PAL APU frame-counter step positions | **Modeled (v2.1.5):** `frame_counter.rs` now clocks the PAL (2A07) sequencer at 8313/16627/24939/33252-33254 (4-step) and 8313/16627/24939/41565-41566 (5-step), region-gated by `FrameCounter::pal` (true only for `Region::Pal`; NTSC/Dendy keep 7457/14913/22371/29828-30, 37281-82). PAL *scheduler* timing (3.2:1, 50 Hz, PAL DMC/noise tables) was already modeled | `pal_apu_tests` (10 sub-ROMs, forced PAL, on-screen verdict via `run_nes_screen`) | **8/10 pass** — the 3 region-independent checks + the 5 PAL frame-counter-timing checks (clock jitter, mode-0/1 length timing, IRQ-flag/IRQ timing) that flipped to PASS with the PAL step positions. NTSC byte-identity preserved (AccuracyCoin APU 141/141, `apu_test` 8/8, NTSC `blargg_apu_2005` 11/11 all unchanged). |
| PAL length halt/reload write-vs-clock ordering | Not modeled: `10.len_halt_timing` / `11.len_reload_timing` need the exact PAL-region priority when a length-counter halt/reload write coincides with a half-frame clock. The PAL step positions advanced these from `FAILED: #2` to `FAILED: #3` / `#4` but did not close them | `pal_apu_tests` `10.len_halt_timing`, `11.len_reload_timing` (forced PAL, on-screen verdict) | **Deferred** (v2.1.5, honestly pinned) — a narrow PAL-specific length-timing residual *adjacent* to the frame-counter step model this line delivered. The NTSC builds of these same two ROMs pass (`blargg_apu_2005` 10 & 11), localizing the gap to a PAL write/clock-collision detail. Pinned fail-loud in `tests/pal_apu_tests.rs`; closing it is a separate deeper length-counter timing investigation |
