...
| `pal_apu_tests` | 10 | 8 | 2 | — | **PAL-region APU oracle (v2.1.5).** blargg's PAL-calibrated APU length/frame-IRQ/timing rebuild, forced to PAL region. These 2005-era ROMs predate the `$6000` protocol (plain NROM, no PRG-RAM → `$6000` reads 0 forever), so the suite decodes the **on-screen** `PASSED`/`FAILED: #<n>` verdict via the `run_nes_screen` harness runner — correcting a prior *false* `$6000`-based oracle that vacuously reported all 10 as passing. **8 strict pass**: the three region-independent checks (`01.len_ctr`, `02.len_table`, `03.irq_flag`) plus the five PAL frame-counter-timing checks (`04.clock_jitter`, `05`/`06.len_timing_mode0`/`1`, `07.irq_flag_timing`, `08.irq_timing`) that flipped to PASS in v2.1.5 when the frame counter gained **region-gated PAL step positions** (2A07 sequencer at 8313/16627/24939/33252-33254 4-step, 8313/16627/24939/41565-41566 5-step). The remaining **2 are fail-loud residual pins** (`10.len_halt_timing` → `FAILED: #3`, `11.len_reload_timing` → `FAILED: #4`; advanced from `#2` by the PAL step positions but not fully closed): a PAL-specific length-counter halt/reload write-vs-half-frame-clock ordering residual, adjacent to the frame-counter step model — documented in `docs/accuracy-ledger.md`. NTSC byte-identity preserved: only `Region::Pal` uses the PAL positions (NTSC/Dendy unchanged; AccuracyCoin 141/141 + `apu_test` 8/8 exact). |
