...
**Current release: v1.0.0 — the production cut.** A cycle-accurate NES/Famicom
...
nestest 0-diff. What ships in v1.0.0:
...
- **Performance + desktop UX** — a display-sync pacing matrix
  ...
  thread, plus a Performance panel and the full debugger UX shell.
...
> current RustyNES release is **v1.0.0**.
...
- **R1 master clock — AccuracyCoin 100.00% (139/139, "no failing tests")**: built
  unconditionally (the `mc-r1-full-cpu` umbrella + its closure are now permanent
  code, not feature flags). nestest 0-diff; blargg cpu_interrupts_v2 **5/5 strict**;
...
**RustyNES version of record:** **v1.0.0** (the production cut). The
...
| `dmc-get-put-scheduler` | `rustynes-apu`, `rustynes-core`, `rustynes-test-harness` | off | Phase 8 Sprint 3 (v1.2.0). Replaces the v1.1.0 phase-agnostic "noop loop + compensating delays" DMC scheduler in `rustynes-core::bus::service_dmc_dma` with Mesen2's canonical get/put cycle alternation model (`NesCpu.cpp:399-447`). Default-off via parallel-implementation pattern (ADR 0007). With the flag ON, AccuracyCoin DMA cluster matches v1.1.0 baseline at **6/10** (4 remaining failures involve the DMC abort path which is NOT yet ported). Promotion to default-on bundled with v2.0 master-clock refactor OR v1.6 fallback patch. The OLD scheduler is preserved as the `#[cfg(not(...))]` branch — default behaviour is bit-identical to v1.1.0. |
| `mc-r1-full-cpu` | `rustynes-cpu`, `rustynes-ppu`, `rustynes-apu`, `rustynes-core`, `rustynes-test-harness` | off | **The v2.0 master-clock umbrella (W3-Stage-4 promotion, 2026-06-10): AccuracyCoin 100.00% (139/139) on this ONE flag.** Composes the R1 floor substrate (substrate + dmc-idle-halt + unified APU clock + one-clock parity + stack/implied dummy reads + the promoted DMC-abort stack) PLUS the Stage-4 fold: `mc-r1-branch-poll-points` (Interrupt-flag-latency), `mc-ppu-2007-render-buffer` (`$2007` Stress), `mc-r1-dmc-delayed-4015` (the delayed-`$4015` status on the unified single-driver DMA engine at the breakthrough parity — DMC+OAM / Explicit + Implicit Abort / Delta-Mod / Implied-Dummy), and `mc-r1-oam-dma-reg-window` (APU Register Activation). nestest 0-diff; cpu_interrupts_v2 5/5 strict; SH\* 6/6; save-state round-trips hold (the gated state is serialized). Scope: NTSC-only (PAL/Dendy frame-structure tests ignored under the flag); the `audio_tests` corpus is default-build-only (R1 changes DMC audio timing by design). Default-build promotion is the later Phase-7/F program. See `docs/audit/v2.0-stage4-promotion-2026-06-10.md`. |

---

## Known residuals (deferred to v2.0 master-clock refactor)

The repo ships **v1.5.0** (2026-05-24). The v1.0.0 90% AccuracyCoin gate
was cleared (90.65%); the residuals below are now deferred to the **v2.0
master-clock-precise scheduling refactor** (see
`docs/audit/gap-analysis-remediation-plan-2026-05-25.md` §3 for the design
and §4 for the per-test closure trajectory). The detailed attempt-log that
follows is retained for historical provenance; it was originally written
against the prioritized sprint backlog at
[`to-dos/phase-6-v1.0.0-final/`](../to-dos/phase-6-v1.0.0-final/overview.md)
(now superseded).
The earlier v1.0.0-rc1 (2026-05-17) deferred them to "v1.x"; rc2 commits
to closing them in v1.0.0 final via the six-sprint plan. Public-facing
rationale + cascade structure: `CHANGELOG.md [1.0.0-rc1]` +
`docs/audit/accuracycoin-readme-analysis-2026-05-17.md`. The C1
IRQ-timing axis has consumed **12 independent rollback attempts** + 1
oracle-only landing (Session-15 / attempt 13) across multiple sessions
without producing an empirical breakthrough. The Session-15 Mesen2
IRQ-cycle oracle (`scripts/mesen2_irq_trace.lua` + cross-diff at
`scripts/irq_trace_cross_diff.py`; baselines under
`crates/rustynes-test-harness/golden/irq_trace/mesen2/`) is permanent
infrastructure for attempts 14+; see `docs/audit/session-15-c1-attempt13-
mesen2-irq-oracle-2026-05-22.md` for the multi-axis-finding rationale and
the Session-16 follow-up sequence.
...
| **post-v1.0 / v2.0** | planned / out of scope | **Shipped:** VRC7 FM audio (v1.1.0), WebAssembly (v1.3.0). **Planned for v2.0:** master-clock-precise scheduling refactor (closes the 4 C1 residuals), FDS, network play, TAS recording. **Out of scope:** Vs. System / PlayChoice-10 arcade variants, pirate / multicart mappers. |
