...
| Document | Topic |
|----------|-------|
| [ai-emulator-provenance-guardrails.md](ai-emulator-provenance-guardrails.md) | **The reference firewall + provenance/attribution ruleset** — ingested into `AGENTS.md` as the top rule; PDF in `ref-docs/`. Reference emulators are black-box oracles (never read their source); `ref-proj/` is removed + firewall-gitignored; if you derive, attribute + keep the license compatible; never launder. |
