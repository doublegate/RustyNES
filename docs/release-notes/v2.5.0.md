...
- **STUN client (RFC 5389)** — `build_binding_request` + `parse_binding_response`
  ...
  live probe.
- **WebRTC/wasm scaffold** — **`rustynes-netplay` now compiles on
...
unit tests; `rustynes-netplay` 41 tests + 1 ignored (incl. the 3/4-player determinism
...
wasm-canvas frontend builds + the new `rustynes-netplay` wasm32 build all compile.
