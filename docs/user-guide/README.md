...
familiar: one ROM per window, cycle-accurate emulation, F-key save states,
hold-to-rewind, and a `~` debugger overlay. Settings live in a TOML file
under your OS's standard config directory.
...
| Enter           | Start |
| Right Shift     | Select |
| F1 / F4         | Save state / Load state (slot 0) |
| F2 / F3         | Reset / Power cycle |
| F5 (hold)       | Rewind |
| `~` (Backquote) | Toggle the debugger overlay |
| Esc             | Quit |

All keys are rebindable. See [Controls](./controls.md).

## A note on ROM legality

RustyNES v2 does not ship commercial Nintendo ROMs and will not. Use only
