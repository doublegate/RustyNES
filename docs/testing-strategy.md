...
| APU PAL (v2.1.5) | `pal_apu_tests/*` (10, forced PAL, on-screen verdict via `run_nes_screen`) | 8/10 strict-pass: `01`/`02`/`03` (region-independent length/table/IRQ-flag) + `04`-`08` (PAL frame-counter-timing, passing since the v2.1.5 region-gated PAL step positions). `10`/`11` (length halt/reload timing) are fail-loud residual pins — a PAL write-vs-clock ordering residual. Corrects a prior `$6000` false oracle (these NROM ROMs have no PRG-RAM). NTSC byte-identity preserved. Documented in `docs/accuracy-ledger.md` |
