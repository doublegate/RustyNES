...
- When relabeling old engine "v2.x" narrative for users, present it as upstream lineage/history — **never as a current RustyNES release version.** The current release is **v1.1.0** (built on the v1.0.0 production core).
- **v1.1.0 is the current release** — the workspace version, CHANGELOG, and docs are bumped; the tag + Linux/macOS-aarch64/Windows binaries + Pages redeploy are the pending final manual gate (do not assume they are live until confirmed). **v1.0.0 remains shipped + live** (tag `v1.0.0`, GitHub release + binaries, the Pages demo + `/api/` docs); its full release + post-release record (the Dependabot `-s ours` integration, the "RustyNES v2" leftover scrub, the combined-Pages + wasm-bindgen fixes, the Actions cleanup) is in `docs/v1.0.0-synthesis-handoff-2026-06-13.md` — read it before touching CI, Pages, or release tooling.
