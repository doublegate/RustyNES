...
- **HD-Pack spatial conditions.** The Mesen `tileNearby` / `spriteNearby` neighbour predicates and the `positionCheckX/Y` + `originPositionCheckX/Y` position checks are now parsed and evaluated (reusing the existing per-pixel tile-source telemetry — the cell position is the array index, neighbours are a relative-cell lookup in the same frame slice). `hd-pack`-gated and output-only, so the core stays byte-identical. (`tileNearby` palette-colour matching and the absolute `tileAtPosition` / `spriteAtPosition` forms remain unsupported.)
