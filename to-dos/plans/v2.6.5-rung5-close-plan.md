...
| | at the version's start | now |
|---|---|---|
| entries executed on both sides | 5 | **146 of 146** |
| entries `NotRun` on one side only | 141 | **0** |
| entries differing | 22 | **5** |

**The acceptance's second clause is met** — *no entry `NotRun` on both sides* —
and its stronger form, every entry executed on both sides, with it. The first
clause, 0 of 146 differing, is not.

Sixteen defects closed, ledger 3.13-3.27 in the sibling's
`docs/oracle-vs-documentation.md`. The five that remain:

| entry | code | what it needs |
|---|---|---|
| `$2007 Stress Test` | 2 | the **ALE / octal-latch** model — one piece of work for all three, and 3.24 records why a bolt-on latch cannot express it |
| `ALE + Read` | 2 | — |
| `Hybrid Addresses` | 2 | — |
