...
shipped tag**. The freed **`v2.3.0`** slot is repurposed as the accuracy/fidelity
...
- **Current release:** **RustyNES v2.3.3 "Cadence"** (2026-08-14) — the display-pacing release, closing the v2.3.x performance campaign. The run-ahead throttle oscillation was traced to a **stale median**: the gate counted 120 frames of a 600-sample ring, so a p50 at index 300 could not leave the previous depth. Expressing the gate in terms of the ring took **6-7 depth changes per 24 s → 1** with spurious releases **2 → 0**; the engage arm then computes instead of waiting, converging a `run_ahead = 3` host in **2.8 s vs 12.1 s** (5/5 paired rounds, exact sign p = 0.0312). Underneath it: compositor refresh via `wp_presentation`, divisor display-sync, and a capture-validity gate that fails closed — dropped frames **135-254 → 1-9**. Frontend-only, so **AccuracyCoin holds at exactly 141/141** and nestest is 0-diff. It does **not** claim the maintainer-reported shudder is resolved; it closes the one *measured* artefact matching that signature. **`docs/STATUS.md` is the authoritative current-state record.**
- **Next up — v2.3.4 (coverage + correctness).** Workstreams: **#360 is closed** — the `movie_ui` recording tests now feed `after_frame` and verify a finished movie against a fresh `Nes`, so v2.3.2's replay attestation is exercised end to end (both the matching and the diverging case). Mapper coverage from [`DEFERRED-AND-CARRYOVER-FEATURES.md`](DEFERRED-AND-CARRYOVER-FEATURES.md) §7 has landed **176 submapper 2** (WAIXING-FS005), **154** (NAMCOT-3453) and **243** (Sachen SA-020A), taking breadth to **174 families**, and fixed a per-game-database defect that had been destroying correct mapper numbers for every Sachen cartridge since v1.2.0. Still open: a measured attempt at **the APU, 18.7% of frame time** — the largest core cost never examined, invisible in the symbol profile because fat LTO inlines it into `cpu_clock`; and a residuals status pass. Expect the APU work to be rejected: the v2.3.1 campaign measured ten core candidates and rejected all ten.
