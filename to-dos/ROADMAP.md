...
- **The mobile apps ship *after* v2.0.0 — the v2.0.1 → v2.1.0 finalization train (maintainer decision, 2026-06-23).** The Android (v1.8.x) and iOS (v1.9.0) apps are held back from their app stores until the v2.0.0 "Timebase" core lands, then finalized and launched **together**: **v2.0.1–v2.0.4** = final Android additions / modifications / enhancements / fixes re-ported onto the v2.0.0 core; **v2.0.5–v2.0.8** = the same iOS finalization (building on the v1.9.0 foundation); **v2.0.9** = true correctness checks + ready-for-release verification for *both* apps + last-minute fixes / testing; **v2.1.0** = the **joint mobile store launch** — flip all release flags and ship **Google Play + Apple App Store + F-Droid** together. Until then the apps continue as **GitHub-sideload** (Android v1.8.x) and **TestFlight** (iOS v1.9.0) only; the old Google-Play-production "Workstream P" collapses into v2.1.0. Full plan: [`plans/v2.0.x-mobile-finalization-plan.md`](plans/v2.0.x-mobile-finalization-plan.md). Rationale: v2.0.0 is the one breaking (save-state / byte-identity) release, so the apps must re-port onto it regardless — shipping after that re-port is the single-launch path.
