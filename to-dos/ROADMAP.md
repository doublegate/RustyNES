...
- **Next up — v2.3.4 (coverage + correctness).** Workstreams: close the attestation test gap (#360, the `movie_ui` recording tests never called `after_frame`, so v2.3.2's replay attestation was exercised by no test); the documented mapper-coverage gaps in [`DEFERRED-AND-CARRYOVER-FEATURES.md`](DEFERRED-AND-CARRYOVER-FEATURES.md) §7; a measured attempt at **the APU, 18.7% of frame time** — the largest core cost never examined, invisible in the symbol profile because fat LTO inlines it into `cpu_clock`; and a residuals status pass. Expect the APU work to be rejected: the v2.3.1 campaign measured ten core candidates and rejected all ten.
