...
**v1.0.0**, and the **v1.1.0 → v1.8.8** feature/platform releases (with
**v1.8.9** now in development) ship on top of it.
...
"Forge"** (+ the `v1.7.1` patch) **→ `v1.8.0` … `v1.8.8` "Atlas"** (the Android
platform train) — the additive, off-by-default feature/platform releases on
that core, of which **`v1.8.8` "Atlas" is the current shipped tag** — with
**`v1.8.9`** now in development. The forward path then targets the real
**RustyNES `v2.0.0` "Timebase"** (the fractional master-clock timebase refactor,
ADR 0002), then the **v2.0.1 → v2.1.0** mobile-finalization train that launches
the Android + iOS apps jointly at **v2.1.0**, and beyond. Where the detailed
sections below carry the inbound engine's
...
- **Current release:** **RustyNES v1.8.8 "Atlas" — the Google-Play launch-readiness release**, the latest in the Android platform train (v1.8.0 … v1.8.8) on the v1.0.0 production core. A cycle-accurate NES/Famicom emulator at the Mesen2 / higan / ares accuracy bar, shipped as a polished desktop application *and* an Android app (AccuracyCoin **100.00% (139/139)**, the 60-ROM `external_real_games` + 52-entry `external_extended` oracles byte-identical, nestest 0-diff). Every feature/platform release since v1.0.0 has been **additive / off-by-default**, so the shipped / native / `no_std` / wasm builds stay byte-identical and AccuracyCoin has held **100% (139/139)** the whole way. Mapper coverage is **168 families** (BestEffort, honesty-gated). The desktop core (v1.7.x "Forge") delivers writable/programmable tools (gated palette/nametable/CHR/OAM/header editors + an inline 6502 assembler), Mesen2-class debugger depth (callstack/step modes + memory-access counters + `.dbg` source maps), a scriptable TAStudio (`tastudio.*` Lua API) + full Lua parity, host IPC/automation behind the off-by-default `script-ipc` feature, tiered Zwinder rewind, expansion-audio NSF routing + MMC5 audio, movie import/export (`.fcm`/`.fmv`/`.vmv`/`.fm2`/`.bk2`), an HD-Pack Builder + the real-Mesen `<tile>`-format loader fix, audio depth (stereo panning / reverb / crossfeed / 20-band EQ / output-device picker), web/wasm parity (browser Lua / File-System-Access API / Gamepad API / PWA-offline / `?settings=` share-links), and an i18n framework (English + Spanish). The **v1.8.x "Android"** train then ports the core to Android (UniFFI `rustynes-mobile` bridge + JNI glue, wgpu SurfaceView + AAudio, SAF ROM loading, a Compose UI, on-screen NES-004 controller, hardware controllers P1–P4, Lua + RetroAchievements + direct-IP/LAN + CGNAT/TURN room-code netplay), culminating in v1.8.8 "Atlas" — Window-Size-Class adaptive layout + edge-to-edge/predictive-back/splash/Material You, EN/ES i18n, a box-art library, Baseline Profiles + R8 full-mode, capture/MP4-clip + PiP/tile/shortcuts/Glance-widget, TV/Leanback + a11y, and Play Games cloud-saves/achievements + Play-Integrity + update/review/vitals (all default-off behind `PGS_ENABLED`/`PLAY_INTEGRITY_ENABLED`). The underlying **v1.0.0 production cut** delivers: the master-clock cycle-accurate core; expansion audio (VRC6/VRC7-OPLL/Sunsoft-5B/Namco-163/MMC5) + Vs./PC10 RGB boards; real-BIOS FDS; 2-4-player rollback netplay (native UDP + browser WebRTC); RetroAchievements (opt-in/native); TAS movie record/replay + save states/rewind; the performance + desktop-UX shell; and a WebAssembly build. The determinism contract holds (DRC is a frontend resampler stage; run-ahead is frontend snapshot/restore orchestration; the core per-frame output is untouched). See `docs/STATUS.md` (single source of truth) + `CHANGELOG.md` `[1.8.8]`…`[1.0.0]`.
...
- **Current state:** **RustyNES v1.8.8 "Atlas" — latest in the Android platform train on the v1.0.0 production core, shipped; v1.8.9 in development.** Every accuracy, compatibility, platform, netplay, RetroAchievements, FDS, Vs/PC10, and performance milestone in the engine-lineage history above is folded into the v1.0.0 core; the v1.1.0 → v1.7.x feature releases then layered (in order) the Lua scripting engine + visual filters/peripherals/devtools/NSF, the library/compatibility/reach pass, the toolchain modernization + Memory-Compare + Vs.-DualSystem detection, the accuracy-and-finish pass, the insight/scriptability/creator-tooling/polish pass, the studio/TAS-tooling/debugger-depth pass, and the writable/programmable-tooling "Forge" pass; the v1.8.x train then ported the whole core to Android. Mapper coverage rose **51 → 168 families** across these releases, all additive / off-by-default, with AccuracyCoin holding **100% (139/139)** the entire time. The engine-lineage version markers (v0.9.x → v2.x) in the bullets above and the phase bodies are upstream history, not RustyNES releases.

**The path to v2.0.0 and beyond:**

- **v1.8.9 (in development — the current dev cycle).** The 13-PR Dependabot consolidation (#180); the **mobile-monetization build-out** (the dormant `rustynes-monetization` crate `.so` + UniFFI bindings wired into the Android build, not yet live — it activates the v2.1.0 freemium model); and a held UX fix. Additive / off-by-default, so AccuracyCoin stays **100% (139/139)** and the shipped builds stay byte-identical. This is the last release before the v2.0.0 timebase rewrite.
- **v2.0.0 "Timebase" — the integer/fractional master-clock timebase refactor (ADR 0002). The real next architectural milestone.** Switch the core timebase from the current "3 PPU dots per CPU cycle" integer-lockstep model to an exact integer **master clock** (ares' NTSC 12:4 and PAL 16:5 ratios; Mesen2's fractional-12-master-clocks-per-CPU-cycle model) so that the per-cycle *phase relationship* between CPU and PPU becomes adjustable. That is the documented prerequisite to close the hard-tier accuracy residuals that 17 prior in-place attempts could not: the C1 IRQ-sample cases (`cpu_interrupts_v2/{2,3,5}` were closed in v1.3.0's re-baseline, but `mmc3_test_2/4` sub-test #3 + the 2 `apu_reset` cases remain), the `$2002` NMI-suppression sub-cycle race, the SH\* internal-bus stores, and the exact PAL 3.2:1 CPU:PPU ratio. This is an **XL / HIGH-risk** structural change (the ADR 0002 stop-condition discipline applies); it is deliberately **deferred from v1.6.0** so the additive feature releases can ship first. Do not conflate it with the *engine-lineage* "v2.0" master-clock work, which is different and already shipped inside the v1.0.0 core. Permanent v2.0 scaffolding already lives in the tree (the `cpu-c1-attempt-17-access-reorder` feature, the `irq_trace` + golden traces, the `M2Phase` per-phase snapshots, the `vbl_race_window_2002_read_sweep` oracle).
- **The iOS app ships across the v1.9.0 → v1.9.9 TestFlight train (interim).** Mirroring the Android v1.8.0 → v1.8.9 arc release-for-release on the v1.0.0 core, the iOS/iPadOS app is built as a 10-release train — **v1.9.0 "Sunrise"** (foundation/MVP: the `rustynes-ios` shim + a SwiftUI shell + the xcframework build, ADR 0026) → v1.9.2 (input + on-screen controller) → v1.9.3 (settings + states + onboarding) → **v1.9.4** (the full wgpu→Metal renderer + WGSL shader stack) → v1.9.5 (power-user feature port) → v1.9.6 (Lua + RetroAchievements + LAN netplay, all reused from the connectivity-complete bridge) → v1.9.7 (CGNAT/TURN netplay + CloudKit save sync) → **v1.9.8 "Horizon"** (platform excellence + the §4.7 compliance pass + privacy manifest + dormant monetization/flavor prep) → **v1.9.9 "Workshop"** (creator/power tools). Each release is additive / off-by-default (AccuracyCoin holds **139/139**), distributed by **TestFlight only** (no App Store until v2.1.0), with §4.7 compliance baked in from day one (no bundled/downloadable ROMs, user-sourced via `UIDocumentPicker`, no Nintendo branding, an ownership notice; ADR 0027). Full plan: [`plans/v1.9.x-ios-train-plan.md`](plans/v1.9.x-ios-train-plan.md).
- **The mobile apps ship *after* v2.0.0 — the v2.0.1 → v2.1.0 finalization train (maintainer decision, 2026-06-23).** The Android (v1.8.x) and iOS (v1.9.x) apps are held back from their app stores until the v2.0.0 "Timebase" core lands, then finalized and launched **together**: **v2.0.1–v2.0.4** = final Android additions / modifications / enhancements / fixes re-ported onto the v2.0.0 core; **v2.0.5–v2.0.8** = the same iOS finalization (building on the v1.9.0–v1.9.9 TestFlight train); **v2.0.9** = true correctness checks + ready-for-release verification for *both* apps + last-minute fixes / testing; **v2.1.0** = the **joint mobile store launch** — flip all release flags and ship **Google Play + Apple App Store + AltStore PAL + F-Droid** together. Until then the apps continue as **GitHub-sideload** (Android v1.8.x) and **TestFlight** (iOS v1.9.0–v1.9.9) only; the old Google-Play-production "Workstream P" collapses into v2.1.0. Full plan: [`plans/v2.0.x-mobile-finalization-plan.md`](plans/v2.0.x-mobile-finalization-plan.md). Rationale: v2.0.0 is the one breaking (save-state / byte-identity) release, so the apps must re-port onto it regardless — shipping after that re-port is the single-launch path.
  - **Mobile monetization (v2.1.0).** An ad-supported freemium model — **AppLovin MAX** + **RevenueCat**, a **$3.99** one-time "Full Version / Remove Ads" unlock (overriding the prior $2.99-no-ads), +11 min × 2 rewarded ads → a 30-minute session, and six premium features. The dormant `rustynes-monetization` crate (wired into the Android build in v1.8.9) activates here.
  - **The `foss` / `play` Android flavor split (ADR 0025).** The Android build splits into a **`foss`** flavor (default — no Google SDKs, no ads, no tracking; the F-Droid + sideload artifact) and a **`play`** flavor (all proprietary SDKs — Billing / Cast / Play Games / Integrity / update-review + AppLovin + RevenueCat — for Google Play). F-Droid requires a Google-/ad-free build, so the flavor split is the only way to reach that channel. The five proprietary subsystems move behind `src/play/` façades (no-op in `src/foss/`); both flavors are verified on-device at v2.0.9.
