...
//! see `rung3-ppu.md` in the sibling repository. The v2.5.6 gate is `$2004` read
//! data on the CPU bus, which is pin-observable; these fields are not.
//!
//! This exists because v2.5.6's gate went red on essentially every `$2004` read
//! (1,284 of 59,993 compared cycles) and two plausible fixes moved the count by
//! six in opposite directions. That is guessing with a gate attached. The v2.5.3
...
fn sprite_evaluation_dot_by_dot() {
    ...
    while nes.frame() < 3 && !nes.is_jammed() {
        nes.run_frame();
    }
    ...
    for r in recs {
        ...
        if prev.as_ref() != Some(&key) {
            println!(
                "  f{} dot{:>3}  n={:>2} m={} found={} sec={:>2} copy={} ovf={} done={} latch={:02X}",
                r.frame,
                r.dot,
                r.sprite_eval_n,
                r.sprite_eval_m,
