...
fn main() -> ExitCode {
    ...
    #[cfg(feature = "mc-r1-dmc-abort-probe")]
    {
        ...
        // Φ-2 hang diagnostic: how many DMAs hit the >60-step escape (GET never
        // fired = wedge), and the put_cycle parity at the hang.
        println!(
            "  Φ-2 HANG: count={} put_cycle_at_hang={} | arms={} get_total={}",
            rustynes_core::rustynes_apu::abort_probe::HANG_COUNT.load(Relaxed),
            rustynes_core::rustynes_apu::abort_probe::HANG_PUT.load(Relaxed),
            rustynes_core::rustynes_apu::abort_probe::ARM_COUNT.load(Relaxed),
            rustynes_core::rustynes_apu::abort_probe::GET_TOTAL.load(Relaxed),
        );

        use rustynes_core::rustynes_apu::abort_probe;
        // W3-Stage-4: how often the consume-edge-quantization suppression
        // latched (expected: ONLY the Implicit $540 X=8/9 restart races).
        println!(
            "  EDGE_SUPPRESS_SET = {}",
            abort_probe::EDGE_SUPPRESS_SET.load(Relaxed),
        );
        // final lever #1: dump the boundary-cycle DMC state captures.
        let nd = (abort_probe::SUBPOS_DIAG_IDX.load(Relaxed) as usize).min(63);
        if nd > 0 {
            println!("  SUBPOS_DIAG blocks (prev2/prev1/cur per block), n={nd}:");
            for i in 0..nd {
                let p = abort_probe::SUBPOS_DIAG[i].load(Relaxed);
                let apu_phase = p & 1;
                let needs = (p >> 1) & 1;
                let buf = (p >> 2) & 1;
                let cannot = (p >> 4) & 0xFF;
                let bytes = (p >> 12) & 0xFFF;
                let bits = (p >> 24) & 0xFF;
                let timer = (p >> 32) & 0xFFFF;
                let tag = match i % 3 {
                    0 => "prev2",
                    1 => "prev1",
                    _ => "CUR  ",
                ...
                println!(
                    "    [{i:>2}] {tag} apu_phase={apu_phase} timer={timer} bits={bits} bytes={bytes} buf={buf} needs_dma={needs} cannot_run={cannot}"
                );
            }
        }
        println!("\n--- explicit-DMC-abort scheduling probe (C-1 diagnostic) ---");
        for (label, v) in abort_probe::snapshot() {
            println!("  {label:<26} {v}");
        }
        // Φ-1: RustyNES $4010=$4E -> first $FFC0 GET offsets (TriCNES = 48).
        let no = (abort_probe::FFC0_OFF_IDX.load(Relaxed) as usize).min(64);
        let offs: Vec<u32> = (0..no)
            .map(|i| abort_probe::FFC0_OFF[i].load(Relaxed))
            .collect();
        println!("  Φ-1 $4E->$FFC0-GET offsets (n={no}, TriCNES=48): {offs:?}");
        // Φ-1 period: $FFC0 GET-to-GET (TriCNES CheckDMATiming region = 576).
        let np = (abort_probe::FFC0_PERIOD_IDX.load(Relaxed) as usize).min(128);
        let pers: Vec<u32> = (0..np)
            .map(|i| abort_probe::FFC0_PERIOD[i].load(Relaxed))
            .collect();
        println!("  Φ-1 $FFC0 reload PERIOD (n={np}, TriCNES=576): {pers:?}");
        // Φ-2: dump all $4000 reads (cycle,value) to CSV for the setup-window diff.
        let r4 = (abort_probe::R4000_IDX.load(Relaxed) as usize).min(4096);
        if let Ok(out) = std::env::var("RUSTYNES_R4000_LOG") {
            if !out.is_empty() {
                let mut s = String::from("cycle,val\n");
                for i in 0..r4 {
                    let p = abort_probe::R4000_LOG[i].load(Relaxed);
                    let _ = writeln!(s, "{},{:02X}", (p & 0xFFFF_FFFF) as u32, (p >> 32) as u8);
                }
                std::fs::write(&out, &s).expect("write r4000");
                println!("  Φ-2 $4000 reads: {r4} -> {out}");
            }
        }
        // Dump the per-$4015-write event log to CSV for trajectory analysis.
        let n = (abort_probe::EVLOG_IDX.load(Relaxed) as usize).min(abort_probe::EVLOG_CAP);
        let mut csv = String::from("idx,cpu_cycle,enable,bytes_remaining,timer\n");
        for i in 0..n {
            let cyc = abort_probe::EVLOG_CYCLE[i].load(Relaxed);
            let meta = abort_probe::EVLOG_META[i].load(Relaxed);
            let enable = (meta >> 31) & 1;
            let bytes = (meta >> 16) & 0x7FFF;
            let timer = meta & 0xFFFF;
            let _ = writeln!(csv, "{i},{cyc},{enable},{bytes},{timer}");
        }
        let out = std::env::var("RUSTYNES_ABORT_EVLOG").unwrap_or_default();
        if !out.is_empty() {
            std::fs::write(&out, &csv).expect("write evlog");
            println!("  evlog: {n} events -> {out}");
        }
        println!();
    }
