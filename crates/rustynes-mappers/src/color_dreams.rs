...
//!
//! A simple mapper used by Color Dreams and Wisdom Tree games.
//! Uses PRG-ROM and CHR-ROM banking.
//!
//! Memory layout:
...
mod tests {
    ...
    #[test]
    fn test_color_dreams_initial_state() {
    ...
    #[test]
    fn test_color_dreams_prg_banking() {
        let rom = create_test_rom(4, 8);
        let mut mapper = ColorDreams::new(&rom);

        // Switch to PRG bank 2
    ...
    #[test]
    fn test_color_dreams_chr_banking() {
        let rom = create_test_rom(4, 8);
        let mut mapper = ColorDreams::new(&rom);

        // Switch to CHR bank 5 (bits 4-7 = 0x50)
        mapper.write_prg(0x8000, 0x50);
        assert_eq!(mapper.read_chr(0x0000), 0x85);
    }

    #[test]
    fn test_color_dreams_combined_banking() {
        let rom = create_test_rom(4, 8);
        let mut mapper = ColorDreams::new(&rom);

        // PRG bank 1, CHR bank 3
    ...
    #[test]
    fn test_color_dreams_chr_ram() {
        let rom = create_test_rom(2, 0);
        let mut mapper = ColorDreams::new(&rom);

        // With no CHR-ROM, should have CHR-RAM
    ...
    #[test]
    fn test_color_dreams_reset() {
        let rom = create_test_rom(4, 8);
        let mut mapper = ColorDreams::new(&rom);

        mapper.write_prg(0x8000, 0x73);
        mapper.reset();
