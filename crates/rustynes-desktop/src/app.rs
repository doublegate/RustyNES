...
impl NesApp {
    ...
    ///
    /// The Console provides pre-converted RGBA data (256x240x4 bytes).
    /// The PPU outputs palette indices, which the Console converts to RGBA
    /// using its internal NES palette lookup table.
    fn update_texture(&mut self, ctx: &egui::Context) {
        // Get pixel data from console or use placeholder
        if let Some(console) = &self.console {
            let rgba_data = console.framebuffer();

            // Copy RGBA data directly from Console (already converted from palette indices)
            // Console.framebuffer() returns 256x240x4 bytes of RGBA data
            let copy_len = self.framebuffer.len().min(rgba_data.len());
            self.framebuffer[..copy_len].copy_from_slice(&rgba_data[..copy_len]);
        } else {
