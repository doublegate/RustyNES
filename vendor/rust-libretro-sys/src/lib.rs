...
/// `struct retro_game_info` from `libretro.h`, defined by hand.
///
/// RustyNES patch (see `VENDORED.md`): the generated bindings reduced this
/// struct to the opaque `{ _address: u8 }`, so `retro_load_game`'s
/// `Some(*game)` copied one byte and no core could read the game it was
/// handed. `build.rs` blocklists the generated version; this definition
/// follows `libretro.h`'s field order and types exactly.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct retro_game_info {
    /// Path to the game, UTF-8; may be NULL when loaded from memory.
    pub path: *const core::ffi::c_char,
    /// The loaded game, or NULL when the core asked for `need_fullpath`.
    pub data: *const core::ffi::c_void,
    /// Size of `data` in bytes.
    pub size: usize,
    /// Implementation-specific metadata string; may be NULL.
