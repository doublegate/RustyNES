...
- **An OAM DMA burst is attributed to its trigger, not its victim.** `STA $4014` only *arms* the transfer; its 513 cycles are stolen from the instructions that follow. The first implementation named the `JMP` that happened to be running. All 256 bytes now name the store that actually caused them.
...
A `.rnm` movie can now carry a rolling hash of its run that anyone else can independently re-derive:
