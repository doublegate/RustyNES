...
**Five ROMs exited 2, and the first diagnosis of why was wrong.** They printed `All N aligned records match` and then a bare `Length mismatch`, which read like pedantry about lengths — so it was written down as a boundary artifact and deferred. It was not. `cpu_boot_trace_diff` now requires the DUT trace to **cover** the reference rather than equal it, and under that rule those five were genuinely **short**: one reference record each, never compared. A small coverage hole wearing a message that read like noise. Five extra cycles on each window closes it — the *same* five for every one, a single systematic boundary rather than per-ROM tuning — and all eleven now exit 0. Demonstrated both ways: the old window still fails as `SHORT`, and a real mutation still fails as a divergence.

The half of the deferral's reasoning that was right is kept: an exit code permanently non-zero on a gate you have decided is fine is an exit code people stop reading, and this release proves it — a pass/fail classifier grepped the tool's prose and matched "Length mismatch" on the substring "match". But the fix for that is to make the exit code **mean** something, not to make the message quieter.
