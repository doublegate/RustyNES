...
echo "==> Building ${CRATE} (release-mobile: release + panic=unwind) for each iOS ABI"
cargo build --profile release-mobile -p "${CRATE}" --target "${TARGET_DEVICE}"
cargo build --profile release-mobile -p "${CRATE}" --target "${TARGET_SIM_ARM}"
cargo build --profile release-mobile -p "${CRATE}" --target "${TARGET_SIM_X86}"

DEVICE_LIB="${ROOT}/target/${TARGET_DEVICE}/release-mobile/${LIB}"
SIM_ARM_LIB="${ROOT}/target/${TARGET_SIM_ARM}/release-mobile/${LIB}"
SIM_X86_LIB="${ROOT}/target/${TARGET_SIM_X86}/release-mobile/${LIB}"
