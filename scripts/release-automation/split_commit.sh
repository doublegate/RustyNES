...
$G restore --staged .
# mapper 119 commit
$G add crates/nes-mappers/src/tqrom.rs crates/nes-mappers/src/lib.rs crates/nes-mappers/src/mmc3.rs crates/nes-test-harness/tests/v21_coverage_mappers.rs docs/mappers.md docs/compatibility.md
$G commit -q -F - <<'EOF'
...
(set = CHR-RAM, clear = CHR-ROM). New nes-mappers/src/tqrom.rs embeds an Mmc3
...
# netplay commit
$G add crates/nes-netplay/src/connection.rs crates/nes-frontend/src/app.rs crates/nes-frontend/src/netplay_ui.rs crates/nes-frontend/src/debugger/netplay_panel.rs
$G commit -q -F - <<'EOF'
...
local-port field now. +2 nes-netplay tests -> 24. No core/chip change:
