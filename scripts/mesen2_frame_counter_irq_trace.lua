-- mesen2_frame_counter_irq_trace.lua
--
-- Focused Mesen2 Lua trace for AccuracyCoin's `Frame Counter IRQ` test.
-- Captures every $4015 / $4017 read and write with per-cycle context
-- (CPU cycle, frame, scanline, dot, value, parity-derived M2 phase).
--
-- Sprint 2 iteration 3 of v1.0.0-final (Session-25, 2026-05-23). Used in
-- conjunction with the matching RustyNES binary `trace_frame_counter_irq`
-- to cross-diff $4015 IRQ-flag clear-on-next-get-cycle semantics between
-- the two emulators. See `docs/audit/session-25-sprint2-iter3-frame-counter-irq-2026-05-23.md`.
--
-- USAGE:
--   xvfb-run -a ~/AppImages/mesen.appimage --testRunner \
--       tests/roms/AccuracyCoin/sub-tests/frame-counter-irq.nes \
--       scripts/mesen2_frame_counter_irq_trace.lua
--
-- ENV VARS:
--   MESEN2_FCIRQ_TRACE_OUT       : output CSV path
--                                  (default /tmp/mesen2_frame_counter_irq_trace.csv)
--   MESEN2_FCIRQ_TRACE_MAX_FRAMES: stop after this many frames (default 60)
--   MESEN2_FCIRQ_TRACE_RESULT_ADDR: stop when this addr becomes non-zero
--                                   (default 0x0467 == result_FrameCounterIRQ)
--
-- CSV schema:
...
-- pre-clear value -- exactly the Test 6/7 axis.

local CONFIG = {
    OUT_PATH    = os.getenv("MESEN2_FCIRQ_TRACE_OUT") or "/tmp/mesen2_frame_counter_irq_trace.csv",
    MAX_FRAMES  = tonumber(os.getenv("MESEN2_FCIRQ_TRACE_MAX_FRAMES") or "60"),
    RESULT_ADDR = tonumber(os.getenv("MESEN2_FCIRQ_TRACE_RESULT_ADDR") or "0x0467"),
}
...
local function open_output()
    ...
    if not f then
        emu.log("[fcirq_trace] could not open " .. CONFIG.OUT_PATH .. ": " .. tostring(err))
        return false
    end
    file = f
    file:write("cpu_cycle,ppu_frame,ppu_scanline,ppu_dot,m2_phase,access,addr,value,irq_pending\n")
    emu.log("[fcirq_trace] writing CSV to " .. CONFIG.OUT_PATH)
    return true
end
...
local function close_output()
    if file then
        file:close()
        file = nil
        emu.log(string.format("[fcirq_trace] wrote %d rows across %d frames", rows, frame_count))
    end
end
...
local function stop_now(reason)
    ...
    stopped = true
    close_output()
    emu.log("[fcirq_trace] STOP: " .. tostring(reason))
    emu.stop(0)
end
...
local function on_4015_write(addr, value)
...
local function on_4015_read(addr, value)
...
emu.addMemoryCallback(on_4015_write, emu.callbackType.write, 0x4015, 0x4015)
emu.addMemoryCallback(on_4017_write, emu.callbackType.write, 0x4017, 0x4017)
emu.addMemoryCallback(on_4015_read,  emu.callbackType.read,  0x4015, 0x4015)
...
emu.log(string.format("[fcirq_trace] armed: max_frames=%d result_addr=$%04X out=%s",
