...
# `rustynes` binary, adapted from Mesen2's `buildPGO.sh`/`PGOHelper`
...
#   The optimized binary lands at target/<triple>/release/rustynes.
#   Compare against the plain release build with
#   `cargo bench -p rustynes-core --bench full_frame` or a frame-time soak.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
cargo pgo build -- -p rustynes-test-harness --bin pgo_trainer
