...
**Current release: v2.1.5 "Fathom" ("Vernier")** (2026-07-11) — the **regression-net & residual** step that opens the **v2.1.5 → v2.2.0 "deepen the existing project"** run, and the current head of the **v2.1.x "Fathom"** accuracy line, which sits on the v2.0.0 "Timebase" one-clock / every-cycle-bus-access scheduler rewrite + Vs. `DualSystem` dual-console support. v2.1.5 lands five workstreams across six PRs (#268/#270, #272, #273, #274, #275) while keeping the NTSC default **byte-identical** — **AccuracyCoin 141/141 (100.00%)**, nestest 0-diff, `blargg_apu_2005` 11/11, the `#![no_std]` chip stack untouched: a **mapper bank-reachability + IRQ regression net** wiring the tepples **Holy Mapperel** ROMs into CI (17 zlib ROMs `insta`-snapshot-pinned; 15/17 detect the correct mapper and reach every bank, the two MMC1 + two FME-7 ROMs surfacing an honestly-pinned WRAM-protection residual); the **first PAL-region APU oracle** (PAL 2A07 frame-counter step positions modeled + blargg `pal_apu_tests` **10/10**, correcting a prior vacuous `$6000` oracle via an on-screen `PASSED`/`FAILED` verdict, and carrying a length halt/reload write-ordering fix that was latent on NTSC too yet stays NTSC-byte-identical); the **MMC3 R1/R2 F5.0 A12-phase instrumentation study** (#274, ADR 0002 refinement — default-off probe, no behavior change, all four residuals stay `#[ignore]`'d); a **measured fat-LTO A/B validation** (#273, retroactively backing the shipped `lto = "fat"` default); and a **real symmetric-NAT netplay production-bug fix** (#272 — the native TURN client now retransmits per RFC 5389 §7.2.1 instead of once). Now on the shipped default the Fathom line keeps the deterministic core byte-identical and layers accuracy + display + quality-of-life work atop the v2.0.0 core: **v2.1.0** accuracy remediation (PPU palette backdrop-override, the mapper-tier completion to **51 Core / 95 Curated / 26 BestEffort of 172**, the MMC3 R1/R2 residual closed by-design-permanent) → **v2.1.1** the Wizards & Warriors game-DB mirroring freeze fix → **v2.1.2 "Prism"** display fidelity (Vs. `DualSystem` desktop second-screen, the NTSC composite-shader ladder, an in-core generated NTSC palette, NSF non-60 Hz + NSFe) → **v2.1.3 "Codex"** quality-of-life (an APU audio filter-model selector, Game Genie code nomination + a full / header-robust code database, a Material-for-MkDocs handbook served at `/docs/` on GitHub Pages) → **v2.1.4 "Caliper"** accuracy hardening (opt-in default-OFF OAM decay modeled on Mesen2, a CI boot-smoke sweep of all 26 BestEffort mapper families, a shared MMC3-clone A12/IRQ timing oracle) → **v2.1.5 "Vernier"** the regression-net & residual step (this release). v2.0.0 "Timebase" remains RustyNES's designated MAJOR-boundary release — see "Timebase (v2.0.0)" below. RustyNES is now a multi-platform emulation suite, all on the one byte-identical cycle-accurate core — **`docs/STATUS.md` is the authoritative current-state record.** What ships beyond the desktop app:
