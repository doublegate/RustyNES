...
**Current release: v2.0.4 "Harbor"** (2026-07-08, an Android release-candidate host-only cut whose core is byte-identical to v2.0.3; the head of the v2.0.x "Harbor" mobile-finalization train that sits atop the v2.0.0 "Timebase" one-clock / every-cycle-bus-access scheduler rewrite + Vs. `DualSystem` dual-console support; the train is preceded by v1.10.0 "Arcade", 2026-07-01). The train's accuracy headline is **v2.0.3**, which promoted the 2-cycle-ALE PPU fetch model to the unconditional default and took AccuracyCoin to a full **141/141 (100.00%)**. v2.0.0 "Timebase" remains RustyNES's designated MAJOR-boundary release — see "Timebase (v2.0.0)" below. RustyNES is now a multi-platform emulation suite, all on the one byte-identical cycle-accurate core — **`docs/STATUS.md` is the authoritative current-state record.** What ships beyond the desktop app:
