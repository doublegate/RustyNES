...
**Current release: v2.1.6 "Fathom" ("Timbre")** (2026-07-11) — the **expansion-audio fidelity** step of the **v2.1.5 → v2.2.0 "deepen the existing project"** run, and the current head of the **v2.1.x "Fathom"** accuracy line, which sits on the v2.0.0 "Timebase" one-clock / every-cycle-bus-access scheduler rewrite + Vs. `DualSystem` dual-console support. v2.1.6 lands across two feature PRs (#277, #279), plus this current-state docs sync, while keeping the **base 2A03 NTSC output byte-identical** — expansion audio is a separate additive `mix_audio` term (0 for non-expansion mappers), so **AccuracyCoin holds 141/141 (100.00%)**, nestest is 0-diff, and `blargg_apu_2005` / `pal_apu_tests` (10/10) / `visual_regression` / the 60-ROM commercial oracle / the `#![no_std]` chip stack are all unchanged: an **expansion-audio decibel oracle** upgrading the bbbradsmith `db_*` comparison ROMs from pure `insta` snapshots into a real accuracy oracle (`audio_expansion.rs` `level_db_*` tests measure the peak amplitude of the reference-2A03-square vs expansion-square segments and **assert** the ratio against the Mesen2 / hardware target — APU triangle ÷ square ≈ 0.524, VRC6 ≈ 1.506, MMC5 ≈ 1.000, N163 1-channel ≈ 6.02; the 19 `insta` snapshots retained as byte-exact guards); **hardware/Mesen2 channel-level calibration** (VRC6 square `256 → 979`, MMC5 pulse/PCM `256/16 → 650/40`, and **Namco 163 `64 → 261`** — the N163 1-channel was **~12 dB too quiet, now fixed**; only the three `db_vrc6a/b` / `db_mmc5` expansion snapshots re-blessed, audio-hash-only); **VRC7 patch-set verification** pinning all 15 melodic (+ 3 rhythm) OPLL instrument patches to the canonical Nuke.YKT dump (+ 5B log-DAC / N163 long-wave unit tests); and a frontend **Audio Mixer** panel (Tools → Audio Mixer — per-source gain sliders + mute, presets, live oscilloscope + VU meters via a determinism-safe `channel_gain` / `channel_mask` overlay that is byte-identical at unity and never serialized into the save state, so save-state / TAS / netplay replay stays byte-identical). **Sunsoft 5B absolute level** and **VRC7 FM level** are honest documented gaps (`docs/accuracy-ledger.md` §Expansion-audio levels), snapshot-guarded rather than faked. The prior step — **v2.1.5 "Vernier"** (2026-07-11) — opened the run with the tepples **Holy Mapperel** mapper bank-reachability / IRQ regression net (17 zlib ROMs `insta`-pinned, two MMC1 + two FME-7 surfacing an honest WRAM-protection residual), the **first PAL-region APU oracle** (`pal_apu_tests` **10/10** + a length halt/reload write-ordering fix that stays NTSC-byte-identical), the MMC3 R1/R2 F5.0 A12-phase study (ADR 0002 refinement, all four residuals stay `#[ignore]`'d), a measured fat-LTO A/B validation, and a real symmetric-NAT TURN-retransmit netplay production-bug fix. Now on the shipped default the Fathom line keeps the deterministic core byte-identical and layers accuracy + display + quality-of-life work atop the v2.0.0 core: **v2.1.0** accuracy remediation (PPU palette backdrop-override, the mapper-tier completion to **51 Core / 95 Curated / 26 BestEffort of 172**, the MMC3 R1/R2 residual closed by-design-permanent) → **v2.1.1** the Wizards & Warriors game-DB mirroring freeze fix → **v2.1.2 "Prism"** display fidelity (Vs. `DualSystem` desktop second-screen, the NTSC composite-shader ladder, an in-core generated NTSC palette, NSF non-60 Hz + NSFe) → **v2.1.3 "Codex"** quality-of-life (an APU audio filter-model selector, Game Genie code nomination + a full / header-robust code database, a Material-for-MkDocs handbook served at `/docs/` on GitHub Pages) → **v2.1.4 "Caliper"** accuracy hardening (opt-in default-OFF OAM decay modeled on Mesen2, a CI boot-smoke sweep of all 26 BestEffort mapper families, a shared MMC3-clone A12/IRQ timing oracle) → **v2.1.5 "Vernier"** the regression-net & residual step → **v2.1.6 "Timbre"** the expansion-audio fidelity step (this release). v2.0.0 "Timebase" remains RustyNES's designated MAJOR-boundary release — see "Timebase (v2.0.0)" below. RustyNES is now a multi-platform emulation suite, all on the one byte-identical cycle-accurate core — **`docs/STATUS.md` is the authoritative current-state record.** What ships beyond the desktop app:
